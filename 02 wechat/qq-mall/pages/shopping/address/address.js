...
Page({
  ...
  selectThis: function (event) {
    ...
    let listData = that.data.addressList.map(function (element, index, array) {
      if (index == itemIndex) {
    ...
    util.request(api.AddressEdit + '?token=' + that.data.token, {
      nId: that.data.addressList[itemIndex].nId,
      detailInfo: that.data.addressList[itemIndex].detailInfo,
      cityName: that.data.addressList[itemIndex].cityName,
      isDefault: that.data.addressList[itemIndex].isDefault,
      ountyName: that.data.addressList[itemIndex].ountyName,
      provinceName: that.data.addressList[itemIndex].provinceName,
      telNumber: that.data.addressList[itemIndex].telNumber,
      userName: that.data.addressList[itemIndex].userName,
      token: that.data.token,
      userId: that.data.userId
    }).then(function (res) {
      if (res.status == 1) {
        if (that.data.orderId != 0) {
          util.request(api.OrderEditAddress + '?token=' + that.data.token, {
