...
Page({
    ...
    getRegionList(regionId) {
        ...
        util.request(api.RegionList, {
            ...
            if (res.status == 1) {
                that.setData({
                    regionList: res.data.map(function(item, index){
                        //标记已选择的
                        if(regionType == item.type && that.data.selectRegionList[regionType - 1].n_ID == item.n_ID)
                {
                    item.selected = true;
                }
            else
    ...
    selectRegionType(event) {
        ...
        //判断是否可点击
        if (regionTypeIndex + 1 == this.data.regionType || (regionTypeIndex - 1 >= 0 && selectRegionList[regionTypeIndex - 1].n_ID <= 0)) {
            return false;
        }
        ...
        this.getRegionList(selectRegionItem.n_ID);
        this.setRegionDoneStatus();
    },
    ...
    setRegionDoneStatus() {
        ...
        let doneStatus = that.data.selectRegionList.every(function(item) {
            return item.n_ID != 0;
    })
        ;
    ...
    selectRegion(event) {
        ...
        if (regionType != 3) {
            ...
            this.getRegionList(regionItem.n_ID);
        } else {
            this.setData({
                selectRegionList: selectRegionList
            });
        ...
        that.setData({
            regionList: that.data.regionList.map(function(item){
                //标记已选择的
                if(that.data.regionType == item.type && that.data.selectRegionList[that.data.regionType - 1].n_ID == item.n_ID)
        {
            item.selected = true;
        }
    else
